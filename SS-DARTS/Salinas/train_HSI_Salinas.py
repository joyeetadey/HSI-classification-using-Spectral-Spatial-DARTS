...
# read data
image_file = '/content/Salinas_corrected.mat'
label_file = '/content/Salinas_gt.mat'

image = sio.loadmat(image_file)
Salinas = image['salinas_corrected']

label = sio.loadmat(label_file)
GroundTruth = label['salinas_gt']

Salinas = (Salinas - np.min(Salinas)) / (np.max(Salinas) - np.min(Salinas))

## data shape
print("data shape: ",Salinas.shape)
...
# Salinas_data = data_wav3

# # Apply PCA for dimensionality reduction
# num_components = 204
# pca = PCA(n_components=num_components, whiten=True)
# Salinas_data_pca = pca.fit_transform(Salinas_data.reshape(-1, Salinas_data.shape[-1]))
# Salinas_data_pca = Salinas_data_pca.reshape(Salinas_data.shape[:-1] + (num_components,))

# print("after PCA",Salinas_data_pca.shape)

# # Apply 3D-Hyperspectral Total Variation denoising
# lambda_tv = 0.1  # regularization parameter
# Salinas_data_denoised = np.zeros_like(Salinas_data_pca)
# for i in range(num_components):
#     Salinas_data_denoised[..., i] = denoise_tv_chambolle(Salinas_data_pca[..., i], weight=lambda_tv, multichannel=False)
# Salinas_data_denoised = torch.from_numpy(Salinas_data_denoised).float()

# # Print the shape of the denoised data
# print("after 3dtv",Salinas_data_denoised.shape)


[nRow, nColumn, nBand] = data_wav.shape
...
def main(seed, cut):
  ...
  for epoch in range(1, args.epochs+1):
    ...
    for iSample in range(nTrain):
        ...
        yy = data_wav[Row[RandPerm[iSample]] - HalfWidth: Row[RandPerm[iSample]] + HalfWidth, \
    ...
    for iSample in range(nValid):
        imdb['data'][:, :, :, iSample + nTrain] = data_wav[Row[RandPerm[iSample + nTrain]] - HalfWidth: Row[RandPerm[
