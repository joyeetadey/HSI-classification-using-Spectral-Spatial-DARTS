...
parser.add_argument('--Train', type=int, default=400, help='Train_num')
parser.add_argument('--Valid', type=int, default=200, help='Valid_num')
...
[nRow, nColumn, nBand] = Salinas.shape
...
def main(genotype, seed, cut):
  ...
  for epoch in range(1, args.epochs+1):
    ...
    for iSample in range(nTrain):
        ...
        yy = Salinas[Row[RandPerm[iSample]] - HalfWidth: Row[RandPerm[iSample]] + HalfWidth, \
    ...
    for iSample in range(nValidate):
        imdb['data'][:, :, :, iSample + nTrain] = Salinas[Row[RandPerm[iSample + nTrain]] - HalfWidth: Row[RandPerm[
    ...
    if epoch == args.epochs:
        ...
        for i in range(numbatch2):
            ...
            for iSample in range(batchva):
                imdb['data'][:, :, :, iSample] = Salinas[Row[RandPerm[iSample + total + i * batchva]] - HalfWidth: Row[RandPerm[
        ...
        heatmap = sns.heatmap(C1, annot=True, cmap='YlOrRd', fmt=".2f", xticklabels=class_labels, yticklabels=class_labels, ax=ax)
        ...
        plt.savefig('/content/result/heatmap_DARTS_V1.png', dpi=300)
