...
parser.add_argument('--num_class', type=int, default=14, help='classes of HSI dataset')
...
# read data
image_file = '/content/Botswana.mat'
label_file = '/content/Botswana_gt.mat'

image = sio.loadmat(image_file)
Botswana = image['Botswana']

label = sio.loadmat(label_file)
GroundTruth = label['Botswana_gt']

Botswana = (Botswana - np.min(Botswana)) / (np.max(Botswana) - np.min(Botswana))

[nRow, nColumn, nBand] = Botswana.shape
...
def main(genotype, seed, cut):
  ...
  for epoch in range(1, args.epochs+1):
    ...
    for iSample in range(nTrain):
        ...
        yy = Botswana[Row[RandPerm[iSample]] - HalfWidth: Row[RandPerm[iSample]] + HalfWidth, \
    ...
    for iSample in range(nValidate):
        imdb['data'][:, :, :, iSample + nTrain] = Botswana[Row[RandPerm[iSample + nTrain]] - HalfWidth: Row[RandPerm[
    ...
    if epoch == args.epochs:
        ...
        for i in range(numbatch2):
            ...
            for iSample in range(batchva):
                imdb['data'][:, :, :, iSample] = Botswana[Row[RandPerm[iSample + total + i * batchva]] - HalfWidth: Row[RandPerm[
        ...
        heatmap = sns.heatmap(C1, annot=True, cmap='YlOrBr', fmt=".2f", xticklabels=class_labels, yticklabels=class_labels, ax=ax)
